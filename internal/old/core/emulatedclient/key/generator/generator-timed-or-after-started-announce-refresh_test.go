...
`
	...
	assert.IsType(t, &TimedOrAfterStartedAnnounceRefreshGenerator{}, generator.IKeyGenerator)
	assert.Equal(t, 1*time.Millisecond, generator.IKeyGenerator.(*TimedOrAfterStartedAnnounceRefreshGenerator).RefreshEvery)
}
