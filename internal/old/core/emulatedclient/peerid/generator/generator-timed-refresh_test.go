...
`
	...
	assert.IsType(t, &TimedRefreshGenerator{}, generator.IPeerIdGenerator)
	assert.Equal(t, 1*time.Millisecond, generator.IPeerIdGenerator.(*TimedRefreshGenerator).RefreshEvery)
}
